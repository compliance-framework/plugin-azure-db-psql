...
func (dp *AzureDataProcessor) Process(policyPaths []string) (proto.ExecutionStatus, error) {
	...
	pager, collectSteps, err := dp.GetAzurePostgresPaginator()
	...
	err = dp.ProcessPaginator(pager, policyPaths, activities)
...
func (dp *AzureDataProcessor) GetAzurePostgresPaginator() (*DBPager, []*proto.Step, error) {
	collectSteps := make([]*proto.Step, 0)

	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		dp.logger.Error("unable to get Azure credentials", "error", err)
		return nil, collectSteps, err
	}
	...
	return pager, collectSteps, err
}

func (dp *AzureDataProcessor) ProcessPaginator(pager *DBPager, policyPaths []string, activities []*proto.Activity) error {
	var accumulatedErrors error

	pagerSteps := make([]*proto.Step, 0)
	startTime := time.Now()

	for pager.More() {
		page, err := pager.NextPage(dp.ctx)
		pagerSteps = append(pagerSteps, &proto.Step{
			Title:       "Get Azure Postgres Databases page",
			Description: "Get Azure Postgres Databases page to loop through all instances.",
		...
		for _, instance := range page.Value {
			...
			for _, policyPath := range policyPaths {
						...
						Props: []*proto.Property{
							{
								...
								Value:   "CCF",
								Remarks: stringAddressed("The Aurora PSQL RDS cluster of which the policy was executed against"),
							},
						},
					},
				}
				...
				policyBundleSteps = append(policyBundleSteps, &proto.Step{
					...
					Description: "Using an instance of a previously collected JSON-formatted Azure Postgres Database configurations, execute the compiled policies",
				})
				...
				for _, result := range results {
					...
					if len(result.Violations) == 0 {
						...
						observation.Description = fmt.Sprintf("Observed no violations on the %s policy within the Azure Postgres Database Compliance Plugin.", result.Policy.Package.PurePackage())
						...
						observation.Description = fmt.Sprintf("Observed %d violation(s) on the %s policy within the Azure Postgres Database Compliance Plugin.", len(result.Violations), result.Policy.Package.PurePackage())
